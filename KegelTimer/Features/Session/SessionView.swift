        ...
        ZStack(alignment: .topTrailing) {
            ...
            VStack(spacing: 40) {
                ...
                ZStack {
                    ...
                    Circle()
                        .trim(from: 0, to: max(appModel.startCountdownProgress, 0.02))
                        .stroke(
                            .white.opacity(0.2),
                        ...
                        .animation(.linear(duration: 0.08), value: appModel.startCountdownProgress)

                    Circle()
                        .fill(.white.opacity(0.4))
                        .frame(width: 18, height: 18)
                        .offset(y: -135)
                        .rotationEffect(.degrees((appModel.startCountdownProgress * 360) + 180))
                        .animation(.linear(duration: 0.08), value: appModel.startCountdownProgress)

                    Text("\(appModel.displayedStartCountdown)")
                        .font(.system(size: 108, weight: .bold, design: .rounded))
                        .foregroundStyle(.white)
                        .monospacedDigit()
                        .contentTransition(.numericText())
                        .animation(.spring(response: 0.28, dampingFraction: 0.82), value: appModel.displayedStartCountdown)
                }
            }
